...
<!-- <template>
  <div>
    <section class="main-container">
      <article>
        <h1 class="main-container__title">
          <NuxtImg
            ...
            title="Manuel Fernández avatar"
          />
          <span class="title-text">{{ t("greeting.hello") }} 👋🏻</span>
        </h1>

        <p>
          {{ t("greeting.me") }}
          <b class="bold-color-text">{{ t("greeting.name") }}</b
          >{{ t("greeting.comma") }}
          {{ t("greeting.job") }}
        </p>

        <p>
          {{ t("greeting.currently") }}
          <b class="bold-color-text">{{ t("greeting.company") }}</b
          >{{ t("greeting.comma") }}
          {{ t("greeting.where") }}
        </p>
        ...
        <ul class="main-container__list">
          <li
            v-for="{ title, description } in t('expertise.list')"
            :key="title"
          >
            <b>{{ title }}</b>
            {{ description }}
        ...
        <h2 class="section-title section-title--link">
          <NuxtLink
            ...
            target="_blank"
            download
            >{{ t("expertise.downloadCV") }}</NuxtLink
          >
        </h2>
      </article>
      ...
      <article class="main-container__article">
        <h2 class="main-container__subtitle">{{ t("projects.title") }}</h2>
